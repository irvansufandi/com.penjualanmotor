...
import com.penjualanmotor.service.AddSend;
import com.penjualanmotor.service.LoginPage;
import com.penjualanmotor.service.UpdateSend;
 ...
 * @author Irvansfd
 */
public class SendPage extends javax.swing.JFrame {
    private ConnectDB Con;
    private ResultSet Res;
    private DefaultTableModel Tab;
    /**
     * Creates new form SendPage
     */
    public SendPage() {
    ...
    private void getSendTable() {
        Object[] Baris = {"ID","Nama Customer","No. Telepon","Tipe Motor","No Plat","Alamat"};
        Tab = new DefaultTableModel(null, Baris);      
        try {
            Res = Con.querySelectWith("pengiriman", "nama_customer LIKE '%"+searchField.getText()+ "%' ORDER BY id_pengiriman ASC");
            while (Res.next()) {
    ...
    private void deleteSend() {
        ...
        String condition = "id_pengiriman = '"+String.valueOf(sendTable.getValueAt(sendTable.getSelectedRow(), 0))+"'";
    ...
    private void getPopUpSend() {
        UpdateSend send = new UpdateSend();
        send.id = sendTable.getValueAt(sendTable.getSelectedRow(), 0).toString();
        send.namaCustomer = sendTable.getValueAt(sendTable.getSelectedRow(), 1).toString();
        send.noTelepon = sendTable.getValueAt(sendTable.getSelectedRow(), 2).toString();
        send.tipeMotor = sendTable.getValueAt(sendTable.getSelectedRow(), 3).toString();
        send.noPlat = sendTable.getValueAt(sendTable.getSelectedRow(), 4).toString();
        send.alamat = sendTable.getValueAt(sendTable.getSelectedRow(), 5).toString();
        send.getDataSend();
    ...
    private void getPrint() {
        ...
        try{   
            ...
            String path = "src/com/penjualanmotor/report/send.jasper";        
    ...
    private void setButton() {
        ...
        editButton.addMouseListener(new MouseListener() {
            ...
            public void mouseClicked(MouseEvent e) {
                ...
                if (sendTable.isRowSelected(row)) {
                    getPopUpSend();     
                }
                else {
    ...
    private void setUI() {
        sendPanel.setBackground(new Color(68, 80, 105));
    ...
    private void initComponents() {
        ...
        sendTable.setModel(new javax.swing.table.DefaultTableModel(
            ...
            new String [] {
                "ID", "Nama Customer", "No. Telepon", "Tipe Motor", "No. Plat", "Alamat"
            }
        ));
        jScrollPane1.setViewportView(sendTable);
        if (sendTable.getColumnModel().getColumnCount() > 0) {
            sendTable.getColumnModel().getColumn(2).setResizable(false);
        }
    ...
    public static void main(String args[]) {
        ...
        try {
            ...
            java.util.logging.Logger.getLogger(SendPage.class.getName()).log(java.util.logging.Level.SEVERE, null, ex);
