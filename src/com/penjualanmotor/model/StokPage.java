...
import com.penjualanmotor.service.AddStok;
import com.penjualanmotor.service.UpdateStok;
 ...
 * @author Irvansfd
 */
public class StokPage extends javax.swing.JFrame {
	...
	DecimalFormat formater = (DecimalFormat) DecimalFormat.getCurrencyInstance();
        DecimalFormatSymbols syimbol = formater.getDecimalFormatSymbols();
        syimbol.setCurrencySymbol("Rp. ");
        syimbol.setMonetaryDecimalSeparator(',');
        syimbol.setGroupingSeparator('.');
        formater.setDecimalFormatSymbols(syimbol);
        return formater.format(value);
    }
    
    public String setYear(String tgl){
      Date tahun = null;
        try {
            tahun = new SimpleDateFormat("yyyy-MM-dd").parse(tgl);
        }
        catch (ParseException e) {
            JOptionPane.showMessageDialog(null, "Data gagal dipanggil! " +e);
        }
        SimpleDateFormat frmt1 = new SimpleDateFormat("yyyy");
        String dateString = frmt1.format(tahun);
        return dateString;
    }
    ...
    public String setDate(String tgl){
      Date tanggal = null;
        ...
        SimpleDateFormat frmt1 = new SimpleDateFormat("dd MMMM yyyy");
        String dateString = frmt1.format(tanggal);
        return dateString;
    }
    
    private void getStokTable() {
        Object[] Baris = {"ID","No. Plat","Tipe Motor","Merek","Kelas","Tahun","Pajak","Harga","Supplier"};
        ...
        try {
            ...
            String conditon = "tipe LIKE '%"+searchField.getText()+"%' ORDER BY id_stok ASC";
            
            Res = Con.fcSelectCommand(column,nameTable,conditon);
            while (Res.next()) {
                Tab.addRow(new Object[] {
                    ...
                    Res.getString(4),
                    Res.getString(5),
                    setYear(Res.getString(6)),
                    setDate(Res.getString(7)),
                    Res.getString(8),
                    Res.getString(9)
                });
            }
            stokTable.setModel(Tab);
        }
    ...
    private void deleteStok() {
        String nameTable = "stok";
        String condition = "no_plat = '"+String.valueOf(stokTable.getValueAt(stokTable.getSelectedRow(), 0))+"'";
    ...
    public void getPopUpStok() {
        UpdateStok updateStok = new UpdateStok();
        updateStok.idMotor = stokTable.getValueAt(stokTable.getSelectedRow(), 0).toString();
        updateStok.noPlat = stokTable.getValueAt(stokTable.getSelectedRow(), 1).toString();
        updateStok.tipeMotor = stokTable.getValueAt(stokTable.getSelectedRow(), 2).toString();
        updateStok.merekMotor = stokTable.getValueAt(stokTable.getSelectedRow(), 3).toString();
        updateStok.kelasMotor = stokTable.getValueAt(stokTable.getSelectedRow(), 4).toString();
        updateStok.tahunMotor = stokTable.getValueAt(stokTable.getSelectedRow(), 5).toString();
        updateStok.pajakMotor = stokTable.getValueAt(stokTable.getSelectedRow(), 6).toString();
        updateStok.hargaMotor = stokTable.getValueAt(stokTable.getSelectedRow(), 7).toString();
        updateStok.supplierMotor = stokTable.getValueAt(stokTable.getSelectedRow(), 8).toString();
        updateStok.getStokData();
    ...
    private void getPrint() {
        ...
        try{   
            ...
            String path = "src/com/penjualanmotor/report/stok.jasper";        
    ...
    private void setButton() {
        ...
        editButton.addMouseListener(new MouseListener() {
            @Override
            public void mouseClicked(MouseEvent e) {
                int row = stokTable.getSelectedRow();
                if (stokTable.isRowSelected(row)) {
                    getPopUpStok();
    ...
    private void setUI() {
        stokPanel.setBackground(new Color(68, 80, 105));
    ...
    private void initComponents() {
        ...
        stokTable.setModel(new javax.swing.table.DefaultTableModel(
            ...
            new String [] {
                "ID", "No. Plat", "Tipe Motor", "Merek", "Kelas", "Tahun", "Pajak", "Harga", "Supplier"
            }
        ));
    ...
    public static void main(String args[]) {
        ...
        try {
            ...
            java.util.logging.Logger.getLogger(StokPage.class.getName()).log(java.util.logging.Level.SEVERE, null, ex);
